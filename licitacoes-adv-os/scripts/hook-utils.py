...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando licitacoes/ acima
- hook_state_load/save    → gerencia licitacoes/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/licitacoes/cowork-state.json."""
    start = start.resolve()
