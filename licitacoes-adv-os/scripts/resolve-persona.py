...
1. Env var LICITAC_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.LICITAC_PERSONA
3. ~/.config/licitacoes-adv-os/active-cowork.json contem persona_path
...
def _try_env_var() -> Path | None:
    p = os.environ.get("LICITAC_PERSONA")
    if p:
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "licitacoes-adv-os" / "active-cowork.json",
        home / ".licitacoes-adv-os" / "active-cowork.json",
    ):
...
def resolve_persona() -> tuple[Path, str]:
    ...
    for fn, tag in (
        (_try_env_var, "env:LICITAC_PERSONA"),
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[licitacoes-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start)\n")
    else:
        sys.stderr.write(f"[licitacoes-adv-os] Persona carregada de {source}: {path}\n")
